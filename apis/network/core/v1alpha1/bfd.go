...
type BFDLinkParameters struct {
	...
	// +optional
	MinTx *uint32 `json:"minTx,omitempty" protobuf:"bytes,2,opt,name=minTx"`
	// MinTx defines the required minimal interval for receiving BFD packets, in msec.
	// +optional
	MinRx *uint32 `json:"minRx,omitempty" protobuf:"bytes,3,opt,name=minRx"`
	// MinEchoRx defines the echo function timer, in msec.
	// +optional
	MinEchoRx *uint32 `json:"minEchoRx,omitempty" protobuf:"bytes,4,opt,name=minEchoRx"`
	// Multiplier defines the number of missed packets before the session is considered down
	// +optional
	Multiplier *uint32 `json:"multiplier,omitempty" protobuf:"bytes,5,opt,name=multiplier"`
	// TTL defines the time to live on the outgoing BFD packet
	// +kubebuilder:validation:Maximum:=255
	// +kubebuilder:validation:Minimum:=2
	// +optional
	TTL *uint32 `json:"ttl,omitempty" protobuf:"bytes,6,opt,name=ttl"`
}
